        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex-1">
            <div className="flex items-center space-x-4 mb-2">
              ...
              <div className="flex items-center space-x-1 px-3 py-1 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300">
                {getStatusIcon(floor.status)}
                <span className="text-sm font-medium">{getStatusText(floor.status)}</span>
              </div>
            </div>
            <p className="text-gray-600 dark:text-gray-400 mb-4">{floor.description.substring(0, 150)}...</p>

            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                <div className="text-sm text-gray-500 dark:text-gray-400">Area</div>
                <div className="font-semibold text-gray-900 dark:text-white">{floor.area.toLocaleString()} sq ft</div>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="font-semibold text-gray-900 dark:text-white">{floor.maxCapacity || 'N/A'}</div>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="card overflow-hidden group hover:shadow-2xl transition-all duration-300">
      <div className="relative h-48 bg-gray-200 overflow-hidden">
        ...
        <div className="absolute top-4 right-4 flex items-center space-x-1 px-3 py-1 rounded-full bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm z-10 shadow-lg border border-white/20 dark:border-white/10 text-gray-900 dark:text-white">
          {getStatusIcon(floor.status)}
          <span className="text-sm font-medium">{getStatusText(floor.status)}</span>
        </div>
      </div>
