  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <motion.div
        ...
        className="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-sm mx-4 transition-colors"
      >
        ...
        <div className="flex justify-end space-x-3">
          <button
            onClick={onCancel}
            className="px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 transition-colors focus:outline-none focus:ring-2 focus:ring-gray-500"
          >
            Cancel
          </button>
          <button
...
const FloorManagement = () => {
  ...
  const SortIcon = ({ field }) => {
    ...
    return sortOrder === 'asc' ? <span className="text-blue-600 dark:text-blue-400 ml-1">↑</span> : <span className="text-blue-600 dark:text-blue-400 ml-1">↓</span>;
  };
  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Floor Management</h1>
          <p className="text-gray-600 dark:text-gray-400">Manage office floors and availability</p>
        </div>
        <button
      ...
      <div className="mb-6">
        <div className="relative max-w-md">
          ...
          <input
            ...
            className="block w-full pl-10 pr-3 py-2 border border-gray-300 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all"
      ...
      <div className="card overflow-hidden">
                  ...
                  {filteredAndSortedFloors.map((floor) => (
                    ...
                    >
                      ...
                      <td className="table-cell">
                        <div>
                          <div className="font-medium text-gray-900 dark:text-white">{floor.name}</div>
                          <div className="text-sm text-gray-500 dark:text-gray-400" title={floor.description}>
                      ...
                      <td className="table-cell">
                        <div className="flex items-center space-x-2">
                          ...
                          <select
                            ...
                            className="text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white border border-gray-300 dark:border-gray-700 rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-primary-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
            ...
            <div className="md:hidden space-y-4 p-4">
              {filteredAndSortedFloors.map((floor) => (
                <motion.div
                  ...
                  className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 space-y-3 hover:shadow-md dark:shadow-black/20 transition-all"
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="font-semibold text-lg text-gray-900 dark:text-white">Floor {floor.floorNumber}</div>
                      <div className="text-sm text-gray-600 dark:text-gray-400">{floor.name}</div>
                    </div>
                    {getStatusBadge(floor.status)}
                  </div>

                  <div className="grid grid-cols-2 gap-3 text-sm border-t border-b border-gray-100 dark:border-gray-700 py-3">
                    <div>
                      <span className="text-gray-500 dark:text-gray-400 block text-xs">Area</span>
                      <div className="font-medium text-gray-900 dark:text-white">{floor.area?.toLocaleString() || 'N/A'} sq ft</div>
                    </div>
                    <div>
                      <span className="text-gray-500 dark:text-gray-400 block text-xs">Price</span>
                      <div className="font-medium text-gray-900 dark:text-white">{floor.formattedPrice}</div>
                    </div>
                  </div>

                  <div className="space-y-2">
                    <label className="text-xs text-gray-600 dark:text-gray-400">Change Status</label>
                    <select
                      value={floor.status}
                      onChange={(e) => handleStatusChangeClick(floor._id, e.target.value)}
                      disabled={updatingStatus === floor._id}
                      className="w-full text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white border border-gray-300 dark:border-gray-700 rounded-lg px-2 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                      aria-label={`Change status for floor ${floor.floorNumber}`}
                    >
                      <option value="available">Available</option>
                      <option value="occupied">Occupied</option>
                  ...
                  <div className="flex items-center space-x-2 pt-2 border-t dark:border-gray-700 gap-2">
                    <button
                      onClick={() => handleEdit(floor)}
                      className="flex-1 px-3 py-2 text-blue-600 dark:text-blue-400 border border-blue-600 dark:border-blue-500 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500"
                      aria-label={`Edit floor ${floor.floorNumber}`}
                    >
                    ...
                    <button
                      onClick={() => handleDeleteClick(floor._id)}
                      className="flex-1 px-3 py-2 text-red-600 dark:text-red-400 border border-red-600 dark:border-red-500 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors focus:outline-none focus:ring-2 focus:ring-red-500"
                      aria-label={`Delete floor ${floor.floorNumber}`}
                    >
        ...
        {!loading && filteredAndSortedFloors.length === 0 && floors.length > 0 && (
          <div className="text-center py-12">
            <HiSearch className="h-16 w-16 text-gray-400 dark:text-gray-600 mx-auto mb-4" aria-hidden="true" />
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">No floors found</h3>
            <p className="text-gray-600 dark:text-gray-400 mb-4">Try adjusting your search term</p>
            <button
              onClick={() => setSearchTerm('')}
              className="text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 font-medium focus:outline-none focus:underline transition-colors"
            >
              Clear search
            </button>
          </div>
        ...
        {!loading && floors.length === 0 && (
          <div className="text-center py-12">
            <HiOfficeBuilding className="h-16 w-16 text-gray-400 dark:text-gray-600 mx-auto mb-4" aria-hidden="true" />
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">No floors found</h3>
            <p className="text-gray-600 dark:text-gray-400 mb-4">Get started by adding your first floor</p>
            <button
              onClick={handleCreate}
              className="btn-primary inline-flex items-center focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 transition-colors"
            >
